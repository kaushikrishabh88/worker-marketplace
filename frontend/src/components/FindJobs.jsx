...
function FindJobs() {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((job) => {
      ...
      const employerName = job.employer?.name?.toLowerCase() || "";
      ...
      const matchesSearch =
        ...
        employerName.includes(search);

      const matchesSkill =
        !selectedSkill || skill === selectedSkill.toLowerCase();
  ...
  }, [jobs, searchTerm, selectedSkill, selectedLocation]);
  ...
  const handleApplyJob = async () => {
    ...
    if (!token || !user) {
      toast.warning("Please login as a worker before applying.");

      return;
    }

    if (user.role !== "worker") {
      toast.warning("Only worker accounts can apply for jobs.");
    ...
    try {
      ...
      setAppliedJobIds((previous) =>
        previous.includes(selectedJob._id)
          ? previous
          : [...previous, selectedJob._id],
      );
  ...
  const selectedJobApplied = selectedJob
  ...
  return (
    <section className="find-jobs" id="find-jobs">
      <div className="find-jobs-heading">
        <span>LOCAL JOB OPPORTUNITIES</span>

        <h2>Find your next opportunity near you.</h2>

        <p>
          Browse jobs posted by local businesses and find work that matches your
          skills.
        </p>
      </div>
      ...
      <div className="job-results-header">
        <div>
          ...
          <h3>Available Jobs ({loading ? "..." : filteredJobs.length})</h3>

          <p>Explore opportunities matching your skills.</p>
        </div>
      ...
      <div className="job-list">
        {loading ? (
          ...
          <div className="job-empty-state">
            <div className="job-empty-icon">⚠️</div>
          ...
          filteredJobs.map((job) => (
            <article className="job-card" key={job._id}>
              <div className="job-card-top">
                ...
                <div>
                  <span className="job-open-badge">OPEN</span>

                  <h3>{job.title}</h3>

                  <p className="job-employer">
                    Posted by{" "}
                    <strong>{job.employer?.name || "Local Business"}</strong>
                  </p>
                </div>
              </div>
      ...
      {selectedJob && (
        <div className="job-details-overlay" onClick={closeJob}>
          <div
          ...
          >
            ...
            <span className="job-details-label">JOB OPPORTUNITY</span>

            <h2>{selectedJob.title}</h2>

            <p className="job-details-company">
              Posted by{" "}
              <strong>{selectedJob.employer?.name || "Local Business"}</strong>
            </p>

            <div className="job-details-meta">
              <span>🛠 {selectedJob.skill}</span>
            ...
            <div className="job-details-salary">
              ...
              <strong>
                ₹{Number(selectedJob.salary || 0).toLocaleString("en-IN")}
              </strong>
            </div>
            ...
            <button
              ...
              disabled={applying || selectedJobApplied}
            >
              {applying
                ? "Applying..."
                : selectedJobApplied
