...
function SentRequests() {
  ...
  const [error, setError] = useState(
    token ? "" : "Please login as an employer to view sent requests.",
  );
  ...
  useEffect(() => {
    ...
    const fetchSentRequests = async () => {
      try {
        ...
        setError(error.message || "Unable to load sent requests.");
      } finally {
        setLoading(false);
      }
    };
  ...
  function handleOpenEdit(request) {
  ...
  async function handleSaveEdit(requestId) {
    if (
      !editData.phone.trim() ||
      !editData.workLocation.trim() ||
      !editData.message.trim()
    ) {
      toast.warning("Phone, work location and message are required.");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/contact-requests/${requestId}`,
        {
          method: "PATCH",
          ...
          body: JSON.stringify({
            ...
            workLocation: editData.workLocation.trim(),
  ...
  return (
    <section className="sent-requests" id="sent-requests">
      <div className="sent-requests-heading">
        <span>HIRING REQUESTS</span>

        <h2>Track your worker requests.</h2>

        <p>
          See which workers you contacted and whether they accepted or rejected
          your request.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="sent-request-empty">
          <div className="sent-request-empty-icon">📤</div>

          <h3>No sent requests yet</h3>

          <p>Requests you send to workers will appear here.</p>
        </div>
      ) : (
        <div className="sent-request-list">
          {requests.map((request) => {
            ...
            let statusMessage = "⏳ Waiting for worker response";

            if (status === "accepted") {
              statusMessage = "✓ Worker accepted your request";
            }

            if (status === "rejected") {
              statusMessage = "✕ Worker rejected your request";
            }

            return (
              <article className="sent-request-card" key={request._id}>
                <div className="sent-request-top">
                  <ProfileAvatar
                    person={request.worker}
                    fallback={request.worker?.emoji || "👨‍🍳"}
                    className="sent-worker-icon"
                    alt={request.worker?.name || "Worker"}
                  />
                  ...
                  <div>
                    ...
                    <h3>{request.worker?.name || "Worker"}</h3>

                    <p>{request.worker?.role || "Worker"}</p>
                  </div>
                </div>
                ...
                <div className="sent-request-date">
                  Sent {new Date(request.createdAt).toLocaleString("en-IN")}
                </div>
                ...
                {status === "pending" && editingRequestId === request._id && (
                  <div className="sent-request-edit-form">
                    <label>
                      ...
                      <input
                        ...
                        disabled={savingRequestId === request._id}
                      />
                    </label>
